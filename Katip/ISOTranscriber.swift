...
protocol ISOTranscriberDelegate {
...
class ISOTranscriber: NSObject {
    ...
    init() {
        super.init()
        self.supportedLocales = Array(SFSpeechRecognizer.supportedLocales())
        self.supportedLocales.sort {
...
extension ISOTranscriber: SFSpeechRecognitionTaskDelegate {
    ...
    func speechRecognitionTask(_ task: SFSpeechRecognitionTask, didFinishRecognition recognitionResult: SFSpeechRecognitionResult) {
        ...
        self.currentSentenceNo = self.currentSentenceNo + 1
        if self.delegate?.shouldStopTranscription() ?? false {
            self.recognitionTask.cancel()
            self.recognitionTask = nil
        }
